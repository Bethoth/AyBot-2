...
module.exports.run = async (client, message, args, argsError) => {
	...
	if(emoji == "Aucun") return argsError("Veuillez mettre un [émoji] ou `liste`.")
	if(emoji == "liste" || emoji == "list") {
		if(message.guild == null) return argsError("Cette sous-commande est uniquement disponible sur serveur.");
		...
		if(!emoji.includes('<')) {
			...
			if(emojiFind == null) return argsError("Cet émoji n'existe pas ou est un émoji de base.");
			id = emojiFind.id;
...
Créé le : **${moment(e.createdAt).format("DD/MM/YYYY** à **hh:mm")}**`);
		...
		} else return argsError("Veuillez mettre un émoji valide.");
	}
}
