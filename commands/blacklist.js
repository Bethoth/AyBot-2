...
module.exports.run = async (client, message, args, argsError) => {
	if(argsError);
	if(args.length < 1) return await argsError("Veuillez mettre une action à faire.");
	if(args.length < 2 && (args[1] != "serveur" || args[1] != "membre")) return await argsError("Veuillez mettre `membre` ou `serveur`.");
	if(args.length < 3 && args[1] == "membre") return await argsError("Veillez mettre un membre.");
	if(args.length < 3 && args[1] == "serveur") return await argsError("Veillez mettre un serveur.");
	let person = args[2];
	let personUser = client.users.get(person);
	let guild = client.guilds.get(args[2]);
	
	if(args[1] !== "membre" && "serveur") return await argsError("Veuillez mettre `membre` ou `serveur`.");
	if(args[0] == "add") {
...
module.exports.help = {
	...
	utilisations: `blacklist add membre/serveur [ID de membre/serveur]\nblacklist remove membre/serveur [ID de membre/serveur]`,
